...
if __name__ == '__main__':
    ...
    e_tripleclouds = EcradIO(input_nc_file=os.path.join(task_path, 'inputs', 'era5_2001-06-10-00.nc'),
                       output_nc_file=os.path.join(task_path, 'mcica', 'pure', 'era5_2001-06-10-00_output.nc'),
                       convert_columns_to_latitude_and_longitude=False,
                       l137_file=summary.l137_file)
    e_mcica = EcradIO(input_nc_file=os.path.join(task_path, 'inputs', 'era5_2001-06-10-00.nc'),
                      output_nc_file=os.path.join(task_path, 'mcica', '52bits', 'era5_2001-06-10-00_output.nc'),
                      convert_columns_to_latitude_and_longitude=False,
                      l137_file=summary.l137_file)
...
#                     l137_file=summary.l137_file)
    ...
    for ax, type_, ax_title in zip(axes, ('lw', 'sw'), ('Longwave', 'Shortwave')):
        ...
        for e in (e_mcica,):
            ...
            x_data_list.append(heating_rate.data)
            y_data_list.append(heating_rate.coord('air_pressure').points / 10**2)
        plot_data_on_mixed_linear_log_scale(fig, ax, x_data_list, y_data_list, label_list, xscale='log',
