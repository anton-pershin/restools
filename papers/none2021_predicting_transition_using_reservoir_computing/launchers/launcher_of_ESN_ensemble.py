...
class ESNTrainAndIntergateGraph(Graph):
    def __init__(self, res, comm, input_filename, task_prefix=''):
        ...
        tt_init, tt_term, train_edge = LocalPythonTimeIntegrationGraph.create_branch(
            comm,
            EsnTrainer(input_filename_key='input_filename',
                       nohup=True,
                       pipes_index_key='pipes_index'),
            edge_need=True
        )#, array_keys_mapping={'input_filename':'input_filenames', 'optimal_esn_filename':'optimal_esn_filenames'})
        ti_init, ti_term, integrate_edge = LocalPythonTimeIntegrationGraph.create_branch(
            comm,
            EsnIntegrator(input_filename_key='input_filename',
                          nohup=True,
                          pipes_index_key='pipes_index'),
            edge_need=True
        )
        ...
        edge_esn_dir.preprocess = make_composite_func(
            make_esn_dirs(data),
            make_little_dump('input_filename',
                             omit=['res'])
        )
        ...
        done_edge.postprocess = make_composite_func(
            make_esn_path(data, input_filename),
            make_little_dump('input_filename',
                             omit=['res'])
        )
        ...
        task_start, task_end = CreateTaskGraph.create_branch(res,
                                                             task_name_maker=task_name_maker)#, array_keys_mapping={'input_filename':'input_filename'})
