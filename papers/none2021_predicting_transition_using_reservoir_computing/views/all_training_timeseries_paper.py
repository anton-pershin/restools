...
#            t_last = t
        ...
        for start_i, end_i, time_shift, mode in zip((ts_data['original_training_begin_time'], ts_data['original_training_end_time']),
            ...
            ax.plot(t, ke, linewidth=2, color='tab:blue')
            ke_last = ke
            t_last = t

        if len(t_last) > 0:
            ax.plot([t_last[-1] + t_last[-1] - t_last[-2], 15000], [ke_last[-1], ke_last[-1]], linewidth=2, color='tab:blue')
#        print(len(t))
        if ts_data['text_location'] == 'left':
            ax.text(0, 20.2, r'$Re = ' + str(ts_data['re']) + r'$', bbox=dict(edgecolor='black', facecolor='white'))
        elif ts_data['text_location'] == 'right':
            ax.text(10000, 4, r'$Re = ' + str(ts_data['re']) + r'$', bbox=dict(edgecolor='black', facecolor='white'))
        #ax.text(7000, 20.2, r'$Re = ' + str(ts_data['re']) + r'$', bbox=dict(edgecolor='black', facecolor='white'))
    ...
    plt.savefig('all_training_timeseries.svg', dpi=200)
