...
def _plot_average_turbulent_profile(ax, task, task_path, label=None, color=None):
    ...
    ax.plot(dUdy, av_turb_field.space.y, linewidth=2, label=label, color=color)
...
if __name__ == '__main__':
    ...
    axes[1].plot([min_freq, max_freq], [unctrl_turb_diss_mean, unctrl_turb_diss_mean],
    ...
    axes[1].set_xlabel(r'$\omega$', fontsize=16)
    axes[1].set_ylabel(r'$Re \times \epsilon$', fontsize=16)
    axes[1].grid()
    ...
    _plot_average_turbulent_profile(axes[0], summary.edge_states_info.task_for_uncontrolled_case, task_path,
                                    color='black')
    axes[0].plot([1., 1.], [-1., 1.], 'k--', linewidth=2)  # laminar solution
    a_i = 3  # A = 0.3
    ...
    axes[0].set_xlabel(r'$dU/dy$', fontsize=16)
